...
- Wi-Fi activates. Device connects to known hotspot from wifi.json. Syncs SPV headers incrementally from last known tip. The screen displays a progress indicator showing sync status: current block height, target height, and estimated time remaining. If the device has been offline for an extended period (more than approximately two weeks), the sync may take several minutes. A "Sync Only" mode is accessible from the home screen to allow pre-session syncing without proceeding to the signing flow.

- Device checks the UTXO cache against the synced block headers. Any new incoming transactions since the last session are detected and recorded in utxo_cache.json. The treasury balance is updated. New incoming transactions are flagged for display on the home screen.
...
- The SD card image (firmware.bin + firmware.sha256 + RELEASE.txt + default config files) is published as a downloadable ZIP from the GitHub releases page. This is the public firmware image. It is identical for every user. It contains no key material. Anyone may download and inspect it.
  To write the firmware image to a blank microSD card, users follow the Firmware Installation Guide published alongside the release. The recommended tool is Win32DiskImager (Windows, free). The process is: download the ZIP, extract firmware.bin, open Win32DiskImager, select Write, select the blank SD card, write the image. This produces a card ready for first-run setup.
  The firmware image and the post-setup card are two entirely different things. The firmware image is public. The post-setup card --- after the setup ceremony has generated and encrypted the key share --- is private and unique to that device holder. These must never be confused.
...
- All N device holders confirm they see the same address. This is the Setup Ceremony. The address is the proof that every key share has been correctly contributed and that the treasury is ready.
  Verifying a full 62-character bech32 address verbally is error-prone. The device therefore displays both the full address and a 6-digit numeric checksum derived from the address (SHA-256 of the address bytes, last 3 bytes interpreted as a decimal number, zero-padded to 6 digits). Device holders compare the 6-digit checksum across devices first --- fast and unambiguous --- then verify the first and last 8 characters of the full address as a secondary confirmation. If any device shows a different checksum or address, setup fails loudly and must restart from the descriptor assembly step.
...
- The device reads the current address index from address_index.json, derives the corresponding P2WSH address from the descriptor, and displays it as a QR code on the screen alongside the full address string and the index number.

- The address index increments and is written back to address_index.json. The next time Receive is opened, a fresh address is shown.

- The payer scans the QR code with their phone wallet and sends the payment. The device holder may also tap Next address to cycle to the following index if they wish to give a different address to a different payer.
...
- Device holder inserts the card, powers on, enters PIN, and connects to Wi-Fi as in a normal session.

- After SPV sync completes the home screen displays the current treasury balance and flags any new incoming transactions with a + prefix and a confirmation count. A confirmed transaction shows a checkmark. An unconfirmed mempool transaction shows a clock symbol.

- The device holder taps the incoming transaction entry to see the full transaction details: amount received, sending address, transaction ID, and confirmation count.
...
> *Human coordination note: the Nostr group channel used for outgoing transaction coordination serves a second purpose for incoming funds. When a device holder sees a new incoming transaction on their screen they should send a plain message to the group: for example Treasury received 0.5 BTC --- 3 confirmations. This is policy not firmware. All holders staying informed about incoming funds without every holder needing to sync their device for every payment is a shared responsibility of the group.*
...
- For version 1.0 running on generic ESP32-S3 hardware, firmware integrity is verified by the SHA-256 hash mechanism described in section 3.3. The device computes the hash of firmware.bin on every boot and compares it against firmware.sha256. A mismatch halts the device before PIN entry. This is the primary firmware integrity mechanism for v1.0 and does not require provisioned hardware.

- For future Wesatoshis-branded hardware with provisioned eFuses, a hardware-backed signature verification path may be added. This would provide stronger attestation than the SD-card hash mechanism but requires device-specific provisioning at manufacture. This is explicitly out of scope for v1.0. Hardware-agnostic operation takes priority over hardware-backed attestation in the first release.

- Any firmware binary whose computed SHA-256 hash does not match the value in firmware.sha256 must cause the device to halt with a red warning screen. The device must not proceed to PIN entry under any circumstances when a hash mismatch is detected.
...
- After a configurable maximum number of failed PIN attempts (default: 10), the card must wipe seed.enc and all treasury state from the SD card. This wipe must be irreversible.
  The firmware-enforced attempt lockout is a convenience protection only. An attacker with physical possession of the SD card can image it and run offline brute-force attacks bypassing the firmware entirely. The true defence against offline brute-force is the PBKDF2 iteration count. The firmware must use a minimum of 600,000 iterations of PBKDF2-HMAC-SHA256 when deriving the encryption key from the PIN, consistent with NIST 2023 guidance. This makes each guess computationally expensive on dedicated hardware. A strong PIN combined with a high iteration count is the correct layered defence. This must be documented prominently in the device holder onboarding guidance.
...
> **OPEN.** PARTIALLY RESOLVED. The firmware ships with two default relays. At least one must be a major independent public Nostr relay not operated by Wesatoshis (e.g. relay.damus.io, relay.nostr.band, or equivalent). This ensures the device functions correctly even if Wesatoshis ceases operations. The specific default relay endpoints are an operational decision; the requirement that at least one is independent is now a formal specification requirement. Self-hosted relay configuration remains a first-class option in the setup flow.
...
> **OPEN.** RESOLVED. HKDF derivation from the descriptor hash is cryptographically sound. All N devices independently derive the same treasury Nostr keypair from the descriptor without any additional out-of-band key exchange. This approach is confirmed as the implementation path for v1.0. No further discussion required.
